...
describe('Testing Functions in ParkingLot class', function() {
    it('Allocates parking without creating a parking slot', function(done) {
        try {
    ...
    it('Slot no. for registration no. KA-01-HH-3141', function(done) {
        var el = parkingLot.getSlotByCarNumber(commands[13]);
        assert.equal(el, 6);
        done();
    });
    ...
    it('Does not park the car with only registration number', function(done) {
        try {
            var ele = parkingLot.parkCar(commands[15]);
        } catch {
            assert.notEqual(ele, 4);
        }
        done();
    });

    it('Does not leave the car greater than the number of slots', function(done) {
        try {
            var ele = parkingLot.leaveCar(commands[16]);
        } catch {
            assert.notEqual(ele, 7);
        }
        done();
